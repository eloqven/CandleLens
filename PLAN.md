...
- **`server-spike` status (implemented):** the Python API (`server/api.py`, `server/indicators.py`, `server/periods.py`, `server/db.py`, `server/ingest.py`, `server/cache.py`) computes multi-slot MA/EMA/WMA across all three period modes, with an LRU cache and server-side run persistence; `src/server/client.ts` + `src/server/serverCompute.ts` map a `RunConfig` to the API; `src/ui/app.ts` + `src/ui/controls.ts` + `src/ui/viewer.ts` drive the full UI (configure → calculate → render → interact → recolor → save/load). Tests: `tests/server/*`. Ingestion is a CLI step (`python api.py --fetch` / `--control`); no live data ingested yet.
